...
class S2S2SEncoder(nn.Module):
    def __init__(self, inp_dim, hidden_dim, n_layers, dropout):
        ...
        self.rnn = nn.LSTM(inp_dim, hidden_dim, n_layers, dropout=dropout, batch_first=True)
...
class Seq2Seq2Seq(nn.Module):
    def __init__(self, lead_dim, horizon_dim, hidden_dim, n_layers,dropout, device):
        super().__init__()
        self.encoder = S2S2SEncoder(inp_dim=4, hidden_dim=hidden_dim, n_layers=n_layers,dropout=dropout)
        self.decoder_1 = S2S2SDecoder(inp_dim=3, hidden_dim=hidden_dim, n_layers=n_layers,dropout=dropout)
        self.decoder_2 = S2S2SDecoder(inp_dim=1, hidden_dim=hidden_dim, n_layers=n_layers,dropout=dropout)
        self.device=device
    ...
    def forward(self, X):
        ...
        for t in range(14):
            ...
            output, hidden, cell = self.decoder_1(X['future_1'][:,t,:].unsqueeze(1), hidden, cell)
        ...
        for t in range(14,self.horizon_dim):
            output, hidden, cell = self.decoder_2(X['future_2'][:,t-14,:].unsqueeze(1), hidden, cell)
...
def train(model, trn_loader, val_loader, optimizer, PARAMS, device, savepath, verbose=True):
...
def main(PARAMS):
    ...
    trn_dataset = w2w_lstm_loader(
        csv_data_path=os.path.join(root, 'wave2web_data',f'{PARAMS["SITE"]}_3zscore.csv'), 
        horizon_days=90, 
        lead_days=60, 
        segment=['trn'],
        site = PARAMS["SITE"],
    ...
    model = train(model, trn_loader, val_loader, optimizer, PARAMS, device=device, savepath=None, verbose=True)
    
    torch.save(model.state_dict(), os.path.join(os.getcwd(),'w2w_results',f'{whole_loader.dataset.site}_weights.pth'))
...
def test_output(model,test_loader, device):
    ...
    results_df = pd.DataFrame(
        ...
        data=data,
        columns = [timedelta(days=ii) for ii in list(range(1,test_loader.dataset.horizon_days+1))]
    )
    ...
    results_df = results_df.loc[results_df['level_0'].isin([timedelta(days=ii) for ii in [1,5,15,30,60,90]])]
    
    results_df = results_df.set_index(['level_0','prediction_dt']).drop(columns='level_1').unstack('level_0')
    ...
    rscores = {
        f'{ii}_days':r2_score(
            results_df.loc[~results_df[['volume_bcm',f'{ii} days']].isna().any(axis=1),'volume_bcm'],
            results_df.loc[~results_df[['volume_bcm',f'{ii} days']].isna().any(axis=1),f'{ii} days']
        ) for ii in [1, 5, 15, 30, 60, 90]
    }
    
    print (test_loader.dataset.site, rscores)
    
    json.dump(rscores, open(os.path.join(os.getcwd(),'w2w_results',test_loader.dataset.site+'_r2score.json'),'w'))
...
def whole_visualise_and_output(model,whole_loader, device):
    ...
    results_df = pd.DataFrame(
        ...
        data=data,
        columns = [timedelta(days=ii) for ii in list(range(1,whole_loader.dataset.horizon_days+1))]
    )
    ...
    plot = results_df.plot(figsize=(20,10))
    fig = plot.get_figure()
    fig.savefig(os.path.join(os.getcwd(),whole_loader.dataset.site+'_wholeset.png'))
    
    results_df.to_csv(os.path.join(os.getcwd(),'w2w_results',whole_loader.dataset.site+'.csv'))    
