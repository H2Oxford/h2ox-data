...
sys.path.append("/Users/tommylees/github/h2ox-wave2web")
...
def main(PARAMS):
    ...
    trn_dataset = w2w_lstm_loader(
        csv_data_path=os.path.join(
            root, "wave2web_data", f'{PARAMS["SITE"]}_zscore.csv'
        ),
        horizon_days=90,
        lead_days=60,
        segment=["trn"],
    ...
    #  "historic" = (n_historical_times, n_historical_features)
    #  "future_1" = (n_forecast_times, n_forecast_features)
    #  "future_2" = (n_future_times, n_future_features)
    ...
    torch.save(
        model.state_dict(),
        os.path.join(
            os.getcwd(), "w2w_results", f"{whole_loader.dataset.site}_weights.pth"
        ),
    )
...
def test_output(model, test_loader, device):
    ...
    json.dump(
        ...
        open(
            os.path.join(
                os.getcwd(), "w2w_results", test_loader.dataset.site + "_r2score.json"
            ),
            "w",
        ),
    )
...
def whole_visualise_and_output(model, whole_loader, device):
    ...
    results_df.to_csv(
        os.path.join(os.getcwd(), "w2w_results", whole_loader.dataset.site + ".csv")
    )
...
if __name__ == "__main__":
    ...
    for kk in ["kabini"]:
        PARAMS = dict(
            ...
            HIDDEN_DIM=32,  #  size of hidden states in LSTM
            LEAD_DIM=60,  #  UNUSED...
            HORIZON_DIM=90,  #  number of days to predict ahead
            DROPOUT=0.5,
        )
