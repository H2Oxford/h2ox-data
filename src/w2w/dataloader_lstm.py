...
class w2w_lstm_loader(Dataset):
        ...
        df = df.append(
            pd.DataFrame(
                index=pd.date_range(
                    ...
                    max_date + relativedelta(days=92),
                    freq="1d",
                ),
                columns=df.columns,
                data=np.nan,
    ...
    def __getitem__(self, index):
        ...
        X_historic = self.df.loc[
            (
                (self.df.index <= idx_dt)
                & (self.df.index > (idx_dt - relativedelta(days=self.lead_days)))
            ),
            historic_cols,
        ].values
        ...
        arrs = [
            ...
            self.df.loc[
                (
                    (self.df.index > idx_dt)
                    & (self.df.index < (idx_dt + relativedelta(days=15)))
                ),
                "sin_dayofyear",
            ].values,
        ]
        ...
        X_future_2 = self.df.loc[
            (
                (self.df.index >= (idx_dt + relativedelta(days=15)))
                & (self.df.index <= (idx_dt + relativedelta(days=self.horizon_days)))
            ),
            "sin_dayofyear",
        ].values.reshape(-1, 1)
        ...
        if self.targets:
            ...
            Y_future_2 = self.df.loc[
                (
                    (self.df.index >= (idx_dt + relativedelta(days=15)))
                    & (
                        self.df.index
                        <= (idx_dt + relativedelta(days=self.horizon_days))
                    )
                ),
                "Y",
            ].values
